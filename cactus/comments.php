...
<form  method="post" action="<?php $this->commentUrl() ?>" id="comment-form">
	...
	<?php _e('ID: '); ?><h5><a href="<?php $this->options->profileUrl(); ?>"><?php $this->user->screenName(); ?></a>. <a href="<?php $this->options->logoutUrl(); ?>" title="Logout"><?php _e('Sign out'); ?> &raquo;</a></h5>
	<div class="vwrap">
		<div class="vheader item3">
		</div>
		<div class="vedit">
			<textarea  name="text" id="veditor" class="veditor vinput" onkeydown="if(event.ctrlKey&&event.keyCode==13){document.getElementById('misubmit').click();return false};" placeholder="Say something?"><?php $this->remember('text',false); ?></textarea>
		</div>
		<div class="vcontrol">
			...
			<button type="submit" title="Cmd|Ctrl+Enter" class="vsubmit vbtn" id="misubmit">Submit</button>
		...
		<div class="vwrap">
		<div class="vheader item3">
			<input name="author" placeholder="Name" class="vnick vinput" type="text" value="<?php $this->remember('author'); ?>" required><input name="mail" placeholder="E-mail" class="vmail vinput" type="email" value="<?php $this->remember('mail'); ?>"<?php if ($this->options->commentsRequireMail): ?> required<?php endif; ?>><input name="url" placeholder="URL(http://)" class="vlink vinput" type="text" value="<?php $this->remember('mail'); ?>"<?php if ($this->options->commentsRequireMail): ?> required<?php endif; ?>>
		</div>
		
		<div class="vedit">
			<textarea  name="text" id="veditor" class="veditor vinput" onkeydown="if(event.ctrlKey&&event.keyCode==13){document.getElementById('misubmit').click();return false};" placeholder="Say something?"><?php $this->remember('text'); ?></textarea>
		</div>
		<div class="vcontrol">
			<div class="col col-20" title="Markdown is supported">
				<a href="https://www.chcst.com/" target="_blank"><svg class="markdown" viewbox="0 0 16 16" version="1.1" width="16" height="16" aria-hidden="true"><path fill-rule="evenodd" d="M14.85 3H1.15C.52 3 0 3.52 0 4.15v7.69C0 12.48.52 13 1.15 13h13.69c.64 0 1.15-.52 1.15-1.15v-7.7C16 3.52 15.48 3 14.85 3zM9 11H7V8L5.5 9.92 4 8v3H2V5h2l1.5 2L7 5h2v6zm2.99.5L9.5 8H11V5h2v3h1.5l-2.51 3.5z"></path></svg></a>
			</div>
			<div class="col col-80 text-right">
	...
	<div class="vinfo" style="display:block;">
		<div class="vcount col">
			<span class="vnum"><?php $this->commentsNum('%d'); ?></span> Comment
		</div>
	</div>
	<?php else: ?>
	<div class="vempty" style="display:block;">Come and be the first to comment~</div>
	<?php endif; ?>
